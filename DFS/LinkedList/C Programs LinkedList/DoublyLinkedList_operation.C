/* 	Write a program to perform the following operations in
	Doubly Linked List  (Insert, Delete, Display, Search)	*/
...
void display(node *tempNode)
{
	...
	return;
     ...
     while(tempNode!=NULL)
     {
	 printf("\n Prev : %d\tSelf : %d\tData : %d\tNext : %d",tempNode->prev,tempNode,tempNode->data,tempNode->next);
	 tempNode=tempNode->next; // Used to move the pointer to the next node
     }

     // Used to reach the last node to traverse the backward
     tempNode=getLastNode(head);
     printf("\n\nBackward List : \n");
     while(tempNode!=NULL)
     {
	 printf("\n Prev : %d\tSelf : %d\tData : %d\tNext : %d",tempNode->prev,tempNode,tempNode->data,tempNode->next);
	 tempNode=tempNode->prev; // Used to move the pointer to the previous node
     }
...
void delet(node *currNode)
{
     ...
     if(currNode==head)
     {
	 if(currNode->next==NULL) // When there is a single node in the list
	 {
		head=NULL;
		return;
	 }
